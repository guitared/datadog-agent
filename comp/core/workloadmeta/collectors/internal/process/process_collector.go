...
func CollectorEventsFromProcessDiff(creations []*processwlm.ProcessEntity, deletions []*processwlm.ProcessEntity) []workloadmeta.CollectorEvent {
	events := make([]workloadmeta.CollectorEvent, 0, len(creations)+len(deletions))
...
func (c *collector) stream(ctx context.Context) error {
	...
	for {
		...
		case diff := <-c.processDiffCh:
			...
			events := CollectorEventsFromProcessDiff(diff.Creation, diff.Deletion)
